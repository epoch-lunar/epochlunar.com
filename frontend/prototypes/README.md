...
| [vfd-column-meter.html](./vfd-column-meter.html) | Segmented bar columns + grid + panel zone ([`docs/brand.md`](../../docs/brand.md)) |
...
| [color-palette.html](../../docs/color-palette.html) | Full design system swatches, fonts, logo treatments — lives in `docs/`, outside the deployed `frontend/` |
...
## Time scale engine — `multi-timescale-clock.html`

`multi-timescale-clock.html` (with `../script.js` and `../time-scales.js`) displays multiple scientific time scales (TAI, UTC, TCG, TCB, GPS, Unix, and experimental TCL) derived from a single canonical timeline. It is an educational visualization of how modern timekeeping systems relate to one another, including the relativistic coordinate time scales used in astrodynamics.
...
The clocks shown on the prototype page are derived from the best available network time using real relationships between time scales.
...
This prototype is intended to make modern scientific timekeeping visible and understandable.
